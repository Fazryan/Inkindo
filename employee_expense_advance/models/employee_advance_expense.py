...
class EmployeeAdvanceExpense(models.Model):
    ...
    @api.multi
    @api.depends('payment_id', 'payment_id.amount')
    def _compute_payed_amount(self):
        for rec in self:
            #rec.paid_amount = rec.move_id.amount
            rec.paid_amount = rec.payment_id.amount
...
#         return [x.id for x in created_moves]
    ...
    @api.multi
    def action_open_journal_payment(self):
        action = self.env.ref('account.action_account_payments')
        res = action.read()[0]
        res['domain'] = str([('id','=',self.payment_id.id)])
        return res
    ...
    def unlink(self):
        for expense in self:
            if expense.state != 'draft':
                raise UserError(_('You cannot delete a expense advance another state draft.'))
        super(EmployeeAdvanceExpense, self).unlink()
